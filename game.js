...
const CUSTOMERS = [
  ...
  { emoji:'👴', moods:['천천히 해도 돼요','맛있게 해주게나','여유 있게 해줘요~'] },
...
function initScene() {
  ...
  // Randomise seated customers at tables occasionally
...
function stopCookingVisual() {
  ...
  clearInterval(state.steamInterval);
  // keep mild steam a moment longer, then clear
...
function selectIngredient(ing, btnEl) {
  ...
  if (!isRequired) {
    ...
    sfx('wrong');
    showFeedback('❌ 재료가 아니에요!', '#e83e3e', 750);
    return;
  }
...
function endGame() {
  ...
  if      (state.score >= 2500) { icon='👨‍🍳'; title='전설의 요리사!';    grade='⭐⭐⭐'; msg='완벽해요! 프로 셰프급이에요. 최고! 🏆'; }
  ...
  else if (state.score >= 600)  { icon='😊'; title='나름 괜찮아요!';    grade='⭐';   msg='좋아요! 재료를 더 빨리 고르면 점수가 올라가요. 🌟'; }
  else                          { icon='😅'; title='요리 견습생!';      grade='🍳';   msg='아직 초보지만 괜찮아요! 레시피를 잘 보고 다시 도전! 🔄'; }
